...
func (w WslConfig) Plan(e *env.Env, o fix.Options) (fix.Plan, error) {
	...
	backup := filepath.Join(filepath.Dir(e.Config.WslConfigPath), fmt.Sprintf(".wslconfig.wsldoctor-%s.bak", p.CreatedAt.UTC().Format("20060102-150405")))
	...
	p.Steps = []fix.Step{
		...
		{Kind: "file_write", Args: []string{e.Config.WslConfigPath, content}, Description: fmt.Sprintf("Comment out %d line(s) in %s with a '# wsldoctor:' reason", len(fixable), e.Config.WslConfigPath)},
		{Kind: "note", Description: "Run wsl --shutdown so the VM picks up the new configuration (wsldoctor fix shutdown)"},
	}
	p.Rollback = []fix.Step{
