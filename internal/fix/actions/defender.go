...
func (d Defender) Plan(e *env.Env, o fix.Options) (fix.Plan, error) {
	...
	p.Steps = []fix.Step{
		...
		{Kind: "note", Description: "Verify by running wslkit doctor from an elevated terminal (DEF001 should report OK). On devices managed by Defender for Endpoint, tamper protection or policy can silently revert local exclusions; DEF001 will show that."},
	}
	p.Rollback = []fix.Step{
