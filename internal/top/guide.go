...
var ReadingGuide = strings.Join([]string{
	section("the VM lines",
		...
		"up is how long the VM has been running: a short one after a long day means it was restarted, by wsl --shutdown, an idle timeout or a crash. swap is the VM's swap file in use; memory the VM had to move there is what makes anonymous memory not quite unreclaimable.",
	...
	section("the rows",
		...
		"LIMIT appears when a row has a cap, from wslkit limit or wslc run --memory: the memory limit that bites first, and the CPU quota. THROTTLED is the share of the time the CPU quota held the row back. DISK USED/FILE is what the distribution's filesystem holds, and the size of its .vhdx file on Windows, which grows and does not shrink by itself; see wslkit disk.",
	),
	section("wslc",
		sessionsNote,
