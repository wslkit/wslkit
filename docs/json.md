...
`storage_vhdx_bytes`. A `host` block carries what Windows charges: `utility_vm`
...
its `containers`, its `host` vmmem and `started_by_measurement`. It is present when the wslc section is: a session
VM is running, wslc is installed, or `--wslc` was given; an empty list means no
session VM was running. With `--wslc` alone, only `host` and `wslc_sessions`
are written, and with `--wsl` alone, no `wslc_sessions`. With
